    ...
    public function getIndex()
    {
        ...
        foreach ($columns_table as $index => $coltab) {
            ...
            if ($join) {
                ...
                if (isset($coltab['join_pk'])) {
                    $joinPk = $coltab['join_pk'];
                    $joinTablePK = $joinPk;
                }
            ...
            $result->where(function ($w) use ($columns_table) {
                foreach ($columns_table as $col) {
                    ...
                    if(config('database.default')=='pgsql'){
                        $w->orwhere($col['field_with'], "ilike", "%".request("q")."%");
                    } else {
        ...
        foreach ($columns_table as $key => $value) {
            if ($value['name'] == 'created_at' || $value['name'] == 'updated_at') {
                if(config('database.default')=='pgsql'){
                    $columns_table[$key]['type_data'] = 'timestamp without time zone';
                } else {
                    $columns_table[$key]['type_data'] = 'timestamp';
                }
            }
        }
    ...
    public function getFindData()
    {
        ...
        if ($q || $id || $table1) {
            ...
            if ($format) {
                ...
                if ($q) {
                    if(config('database.default')=='pgsql'){
                        $rows->whereraw("CONCAT($format) ilike '%".$q."%'");
                    } else {
                ...
                if ($q) {
                    if(config('database.default')=='pgsql'){
                        $rows->where($orderby_table.'.'.$orderby_column, 'ilike', '%'.$q.'%');
                    } else {
