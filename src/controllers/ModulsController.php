...
use muhammadfahrul\crudbooster\fonts\Fontawesome;
...
class ModulsController extends CBController
{
    ...
    public function getStep1($id = 0)
    {
        ...
        return view("crudbooster::module_generator.step1", compact("tables_list", "connection_list", "fontawesome", "row", "id"));
    }
    ...
    public function postStep2()
    {
        ...
        if (! Request::get('id')) {
            ...
            $id = DB::table($this->table)->insertGetId(compact("controller", "name", "table_name", "connection", "icon", "path", "created_at"));
    ...
    public function getStep3($id)
    {
        ...
        $types = [];
        foreach (glob(base_path('vendor/muhammadfahrul/crudbooster/src/views/default/type_components').'/*', GLOB_ONLYDIR) as $dir) {
            $types[] = basename($dir);
        }
    ...
    public function getTypeInfo($type = 'text')
    {
        ...
        echo file_get_contents(base_path('vendor/muhammadfahrul/crudbooster/src/views/default/type_components/'.$type.'/info.json'));
    }
