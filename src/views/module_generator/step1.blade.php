        ...
        <div class="box-body">
            <form method="post" action="{{Route('ModulsControllerPostStep2')}}">
                ...
                <div class="form-group">
                    ...
                    <select name="connection" id="connection" required class="select2 form-control" value="{{$row->connection}}">
                        <option value="">{{cbLang('text_prefix_option')}} Connection</option>
                        @foreach($connection_list as $connection)

                            <option {{($connection == $row->connection)?"selected":""}} value="{{$connection}}">{{$connection}}</option>
