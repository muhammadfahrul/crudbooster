...
@if($columns)
    @push('bottom')
        ...
        <script>
            $(function () {
                $('#advanced_filter_modal').removeAttr("tabindex");  
                        ...
                        <div class="modal-body">
                            ...
                            <div class='form-group'>
                                ...
                                <div class='row-filter-combo row'>
                                    ...
                                    <div class='col-sm-9'>
                                        @if (isset($col["select_option"]))
                                        <select class='form-control select2' style="width: 100%;" name='filter_column[{{$col["field_with"]}}][value]'>
                                            <option value=''>Choose {{$col["label"]}}</option>
                                            @foreach ($col["select_option"] as $item)
                                            <option {{ (CRUDBooster::getValueFilter($col["field_with"]) == $item->{$col["field_raw"]})?"selected":"" }} value='{{ $item->{$col["field_raw"]} }}'>{{ $item->{$col["field_raw"]} }}</option>
                                            @endforeach
                                        </select>
                                        @else
