<div class='form-group {{$header_group_class}} {{ ($errors->first($name))?"has-error":"" }}' id='form-group-{{$name}}' style="{{@$form['style']}}">
    ...
    <div class="{{$col_width?:'col-sm-10'}}">
        ...
        if (@$form['datatable']):
            ...
            $selects_data = DB::table($tables[0])->select($tables[0].".".$joinPk);
            ...
            if (@$form['datatable_where_in'] && @$form['join_pk']) {
                $selects_data->whereIn($form['join_pk'], $form['datatable_where_in']);
            }
            ...
            if (count($tables)) {
                for ($i = 1; $i <= count($tables) - 1; $i++) {
                    ...
                    $selects_data->leftjoin($tab, $tab.'.'.$joinPk, '=', 'id_'.$tab);
                }
            }
            ...
            if ($form['relationship_table']) {
                ...
                foreach ($selects_data as $d) {
                    ...
                    $checked = (is_array($value) && in_array($d->{$joinPk}, $value)) ? "checked" : "";
                    // echo "
                    ...
                    echo "
												<div data-val='$val' class='checkbox $disabled'>
												  <label>
												    <input type='checkbox' $disabled $checked name='".$name."[]' value='".$d->{$joinPk}."'> ".$d->{$datatable_field}."								    
												  </label>
												</div>";
                }
            } elseif ($form['value_primary']) {
                @$value = explode(';', $value);
                ...
                foreach ($selects_data as $d) {
                    ...
                    echo "
												<div data-val='$val' class='checkbox $disabled'>
												  <label>
												    <input type='checkbox' $disabled $checked name='".$name."[]' value='".$d->{$joinPk}."'> ".$val." 								    
												  </label>
												</div>";
                }
            } else {
